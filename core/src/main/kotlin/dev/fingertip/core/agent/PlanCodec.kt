...
object PlanCodec {
    ...
    fun buildPrompt(request: PlanRequest, maxHistory: Int = 12): String = buildString {
        ...
        appendLine("""  {"action":"captureAll","selector":{...},"as":"<name>","limit":50}""")
        appendLine("     captureAll reads EVERY match into one value, plus <name>_count.")
        appendLine("     Use it to total, compare or summarise a list in one step.")
